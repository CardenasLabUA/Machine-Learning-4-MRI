clear; close all; clc;
folder_path = 'D:\Users\Joey D\Desktop\Joey\GitHub\Machine-Learning-4-MRI\Imaging_of_pH\3-Oxilan\Code\ML_python\Oxilan\Images\';
...
for p=1:n_pH
    ...
    t2_img = reshape(Bruker_reader([folder_path,pH_folders(p).name,'\2']),128,128,50);
    ...
    t2_img=reshape(t2_img,[],50);
    centered=nan(length(indices),101);
    uncentered=centered;
    ppms=centered;
    d_pars=nan(length(indices),9);
    rsq=nan(length(indices),1);
    pH=rsq;
    d_conc=rsq;
    d_t2=rsq;
    d_t2_rsq=rsq;
    d_t2_curve=nan(length(indices),47);
    ...
    for q=1:length(indices)
        ...
        d_pars(q,2)=fit.pars(5);
        d_pars(q,3)=fit.pars(8);
        d_pars(q,4)=fit.pars(1);
        d_pars(q,5)=fit.pars(4);
        d_pars(q,6)=fit.pars(7);
        d_pars(q,7)=fit.pars(3);
        d_pars(q,8)=fit.pars(6);
        d_pars(q,9)=fit.pars(9);
        ...
        te_fit=te(4:end);
        d_t2_curve(q,:)=s;
        Fittedpars=fit_MSME(te_fit,s)'; %#ok<*AGROW>
        d_t2(q,1)=Fittedpars(1);
        d_t2_rsq(q,1)=Fittedpars(4);
    ...
    t2_curve=[t2_curve;d_t2_curve];
    t2=[t2;d_t2];
    t2_rsq=[t2_rsq;d_t2_rsq];
    cd(here);
    save('all_voxel_centered_spectra_with_T2.mat','xpred','c_centered','c_rsq','c_pH','c_conc','c_pars','te_fit',...
        'c_ppm','c_uncentered','t2_curve','t2','t2_rsq','pHs','concs','-v7');
    cd(folder_path);
    p
end
...
%%
folder_path = 'D:\Users\Joey D\Desktop\Joey\Sp 2017\Oxilan\Averages\PBS\CEST\';
% Set up different pHs
here=pwd;
center_range = [-8,15];
cd(folder_path);
...
cd(here);
save('all_voxel_centered_spectra_with_T2.mat','xpred','c_centered','c_rsq','c_pH','c_conc','c_pars','te_fit',...
        'c_ppm','c_uncentered','t2_curve','t2','t2_rsq','pHs','concs','c_dif','-v7');
...
for p=1:8
    subplot(4,2,p); hold on;
    c_plot=t2_curve(c_rsq>rsq_cut,:);
    c_plot=c_plot(c_pH(c_rsq>rsq_cut,:)==pHs(p),:);
    for j=1:size(c_plot,1)
        plot(te_fit,c_plot(j,:)); ylim([0,1]); xlim([0,.5]);
    end
    hold off;
end
...
for p=1:length(pHs)
    rmse_pH(p)=(sum((response(c_pH==pHs(p))-yfit(c_pH==pHs(p),1)).^2)./ length(response(c_pH==pHs(p))))^(1/2) ./ mean(response(c_pH==pHs(p)))*100;
end
...
[a_centered,xpred,a_pH,a_conc,a_ppm,a_rsq,a_signal]=fit_CEST_all_pH(data_folder,expected_cest_range);
