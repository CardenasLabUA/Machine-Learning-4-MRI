...
data_folder = 'F:\GitHub\Machine-Learning-4-MRI\Imaging_of_pH\3-Oxilan\Data\';
expected_cest_range = [-8,15];
[c_centered,xpred,pH,conc,c_ppm,c_rsq,c_signal]=fit_CEST_all_pH(data_folder,expected_cest_range);
...
% [~,~,~,~,beta_pH,~] = plsregress(c_centered,pH,Components(j),'CV',size(c_centered,1));
% beta_pH_var(:,j)=beta_pH;
% % yfit = [ones(size(c_centered,1),1) c_centered]*beta_pH;
% [~,~,~,~,beta_conc,~] = plsregress(c_centered,conc,Components(j),'CV',size(c_centered,1));
% beta_conc_var(:,j)=beta_conc;
% end

[~,~,~,~,beta_pH,~] = plsregress(c_centered,pH,Components,'CV',size(c_centered,1));
% yfit = [ones(size(c_centered,1),1) c_centered]*beta_pH;
[~,~,~,~,beta_conc,~] = plsregress(c_centered,conc,Components,'CV',size(c_centered,1));
...
%print(['pH_map_',num2str(chosen_pH)], '-dtiff', '-r300');
...
%print(['conc_map_',num2str(chosen_pH)], '-dtiff', '-r300');