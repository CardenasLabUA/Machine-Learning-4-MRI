clear; close all; clc;
folder_path = 'D:\Users\Joey D\Desktop\Joey\GitHub\Machine-Learning-4-MRI\Imaging_of_pH\3-Oxilan\Code\ML_python\Isovue\Images\';
center_range = [-5,10];
...
for p=1:n_pH
    ...
    for q=1:length(indices)
        ...
        d_pars(q,1)=fit.pars(1); % Water amp
        d_pars(q,2)=fit.pars(5); % Water width
        d_pars(q,3)=fit.pars(9); % Water offset
        d_pars(q,4)=fit.pars(3); % hydroxyl amp
        d_pars(q,5)=fit.pars(7); % hydroxyl width
        d_pars(q,6)=fit.pars(11); % hydroxyl offset
        d_pars(q,7)=fit.pars(2); % isovue 4.2 amp
        d_pars(q,8)=fit.pars(6); % isovue 4.2 width
        d_pars(q,9)=fit.pars(10); % isovue 4.2 offset
        d_pars(q,10)=fit.pars(4); % isovue 5.6 amp
        d_pars(q,11)=fit.pars(8); % isovue 5.6 width
        d_pars(q,12)=fit.pars(12); % isovue 5.6 offset
    ...
    cd(here);
    save('all_voxel_centered_spectra_no_T2_isovue.mat','xpred','c_centered','c_rsq','c_pH','c_conc','c_pars',...
...
for p=1:8
    ...
    for j=1:size(c_plot,1)
        plot(c_plot_ppm(j,:),c_plot(j,:)); ylim([0,1]); xlim([xpred(1),xpred(end)]);
    end
    title(['pH = ',num2str(pHs(p))]);
    hold off;
end
...
[a_centered,xpred,a_pH,a_conc,a_ppm,a_rsq,a_signal]=fit_CEST_all_pH_iso(data_folder,expected_cest_range);
