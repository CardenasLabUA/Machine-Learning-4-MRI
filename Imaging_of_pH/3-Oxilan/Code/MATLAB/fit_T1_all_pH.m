function [T1,T1_error,T1_rsq,Signal,TR]=fit_T1_all_pH( folder_path )
%% T1
here=pwd;
cd(folder_path)
pH_folders=dir('pH*');
...
last_n=0;
for k=1:n_pH
    cd([folder_path,pH_folders(k).name,'\T1']);
    ...
    n_conc=length(conc_files);
     ...
     for q=1:n_conc
        ...
        Signal(q+last_n,:)=s;
        Fittedpars=fit_variable_TR(TR,s./max(s)); %#ok<*AGROW>
        T1(q+last_n,1)=Fittedpars(1);
        T1_error(q+last_n,:)=Fittedpars(2:3);
        T1_rsq(q+last_n,:)=Fittedpars(4);
     end
